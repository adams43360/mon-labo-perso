...
if (hasProxyEnv) {
  ...
  // `requestTls` s'applique à la connexion TLS tunnelée vers la cible (via CONNECT),
  // c'est elle qui échoue avec SELF_SIGNED_CERT_IN_CHAIN si le CA du proxy MITM
  // n'est pas explicitement approuvé.
