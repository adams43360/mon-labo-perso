...
/** Rapport markdown prêt à coller dans l'issue GitHub de l'expérience. */
export function toMarkdown(run: RunResult): string {
  ...
  const lines: string[] = [
    ...
    "",
    `_Run du ${run.date} · harness maison (Vitest) · ${results.length} cas_`,
    "",
    `| Métrique | Score |`,
    `|---|---|`,
  ...
  if (errors.length === 0) {
    ...
    lines.push(
      ...
      ...errors.map(
        (e) =>
          `| #${e.id} | ${e.difficulte} | ${e.expected} | ${e.predicted ?? "inclassable"} | \`${e.raw.slice(0, 60)}\` |`,
      ),
    );
  }
  return lines.join("\n") + "\n";
